...
def generate_vector():
    ...
    sad_h = []
    sad_n = []
    path_dir = os.listdir('data\\train\\happy')
    for file in path_dir:
        full_name = os.path.join('data\\train\\happy', file)
    ...
    path_dir = os.listdir('data\\train\\normal')
    for file in path_dir:
        full_name = os.path.join('data\\train\\normal', file)
        with open(full_name, 'r') as f:
            reader = csv.reader(f)
            vector = [float(row[0]) for row in reader]
            vector = getattr.get_vector(vector)
    ...
    path_dir = os.listdir('data\\train\\sad')
    for file in path_dir:
        full_name = os.path.join('data\\train\\sad', file)
        with open(full_name, 'r') as f:
            reader = csv.reader(f)
            vector = [float(row[0]) for row in reader]
